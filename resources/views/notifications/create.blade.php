    ...
    <div class="bg-white rounded-3xl md:rounded-2xl border md:border border-gray-100 md:border-gray-200 p-6 max-w-2xl shadow-sm md:shadow-none">
        <form method="POST" action="{{ route('notifications.store') }}" class="space-y-6"
              x-data="notificationForm(
                  ...
                  {!! \Illuminate\Support\Js::from(old('times', ['09:00'])) !!},
                  {!! \Illuminate\Support\Js::from(array_map('intval', (array) old('week_days', []))) !!},
                  {{ old('every_n_days', 1) }},
                  {{ old('cyclical_value', 1) }},
                  '{{ old('cyclical_unit', 'weeks') }}'
              )">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" required
                       placeholder="e.g. Take vitamins, Weekly review, Pay rent"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="2"
                          ...
                          class="w-full px-3.5 py-2.5 text-sm rounded-lg border border-gray-300 transition-colors outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <div x-show="scheduleType === 'every_n_days'" x-cloak class="flex items-center gap-3">
                    ...
                    <input type="number" name="every_n_days" min="1" max="365"
                           x-model="everyNDays"
                    ...
                    @error('every_n_days') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                ...
                <div x-show="scheduleType === 'cyclical'" x-cloak class="flex items-center gap-3">
                    ...
                    @error('cyclical_value') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div x-show="scheduleType !== 'as_needed'" x-cloak>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    Times
                    <span class="text-gray-400 font-normal">(optional — defaults to 09:00)</span>
                </label>
                <div class="space-y-2">
                ...
                @error('times.*') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>
